...
fdibuix<-function(X,useonly=1:nrow(X),v=1:ncol(X),npart=ncol(X),f,scale=1,conf=0.68,main="clrCanVarPlot",labpart=colnames(X)){
  # data
  X=X[useonly,v]
  f=f[useonly]
  fcanpobClr(X,f,doplot=TRUE,npart=npart,scale=scale,conf=conf,main=main,labpart=labpart)  
...
fcanpobClr<-function(X,g,doplot=FALSE,npart=ncol(X),scale=1,conf=0.68,main="clr-CanVarPlot",labpart){
  ...
  if (doplot) {
    ...
    # plot vectors
    fvectors(cso[useonlyvec,], gmYclr, col="blue", cex=1.1, lwd=2,labels=labpart[useonlyvec])
